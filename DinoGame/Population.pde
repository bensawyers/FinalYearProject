...
class Population {
  ...
  private HashMap<float[], ArrayList<Float>> avgQTable = new HashMap<float[], ArrayList<Float>>();
  private HashMap<float[], Float> finalQTable = new HashMap<float[], Float>();
  ...
  Population(int size, HashMap<float[], Float> table) {
      for (int i =0; i<size; i++) {
        pop.add(new Player(table));
      }
  }
  //------------------------------------------------------------------------------------------------------------------------------------------
  ...
  void updateReinforce() {
    for (int i = 0; i< pop.size(); i++) {
      if (!pop.get(i).dead) {
        float[] cState = pop.get(i).getState();
        float playerAction = pop.get(i).chooseAction(cState);
        pop.get(i).act((int)playerAction);
        pop.get(i).setCurrentState(cState);
        pop.get(i).setPrevAction(playerAction);
        //move the player to reflect the chosen action
        pop.get(i).update();
        pop.get(i).show();
      }
    }
    updateObstacles(); //<>//
    for (int i = 0; i< pop.size(); i++) {
      //after the players position and the obstacles have been updated calculate the Q-value
      //for the previous state
      pop.get(i).rewardPlayer(pop.get(i).getCurrentState(), pop.get(i).getPrevAction());
    }
  }
  
  void populateAvgQTable(){
    avgQTable = new HashMap<float[], ArrayList<Float>>();
    for (int i = 0; i< pop.size(); i++) {
      HashMap<float[], Float> pTable = pop.get(i).getQTable();
      
      //get the Q-values for each of the state-action pairs and add
      //them to a hashmap to be averaged
      for(float[] pKey: pTable.keySet()){
        for(float[] avgKey: avgQTable.keySet()){
          if(Arrays.equals(pKey, avgKey)){
            ArrayList<Float> valsToAvg = avgQTable.get(avgKey);
            valsToAvg.add(pTable.get(pKey));
          }else{
            ArrayList<Float> valsToAvg = new ArrayList<Float>();
            valsToAvg.add(pTable.get(pKey));
          }
        }
      }
  ...
  //average the values for the state-action pairs to be put into the global Q-table
  void averageValues(){
    for(float[] avgKey: avgQTable.keySet()){
      ArrayList<Float> valsToAvg = avgQTable.get(avgKey);
      float sum = 0;
      for(float vals : valsToAvg){
        sum += vals;
      }
      sum = sum/valsToAvg.size();
      sum = (float)(Math.round(sum*100d)/100d);
      finalQTable.put(avgKey, sum);
    }
  }
